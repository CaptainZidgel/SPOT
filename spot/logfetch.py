...
class Fetcher:
    ...
    * Presets: "6s" is the above lambda (this is not a replacement for using an Approver)
    ...
    def from_dir(self, limit=None, sort=False, select=None): #Limit mostly exists here for my own testing purposes so I don't have to read 8k logs
        l = []
        ...
        for file in opts:
                with open(os.path.join(self.save_directory, file), 'r', encoding="utf8") as f:
                        _id = file.split("_")[1].split(".")[0]
                        if select == None or int(_id) in select: #This has regrettable speed implications... takes almost 1 minute longer depending on the range.
                                log = json.load(f) #Apparently waiting to load a file until a check succeeds doesn't actually do much in the way of increasing speed.
                                log["id"] = _id
                                l.append(log)
        if sort:
